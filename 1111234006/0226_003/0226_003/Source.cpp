...
int main(void)
{
    ...
    unsigned int inputID; // 宣告一個名為 inputID 的變數，它的資料型別是 unsigned int
    int found = 0; // 宣告一個名為 found 的整數變數，並將其初始化為 0
    ...
    // 輸入學號
    printf("Enter student ID to search: "); //會顯示一個提示訊息，要求使用者輸入學生 ID
    scanf_s("%u", &inputID); // 會從鍵盤讀取一個正整數並將其存入 inputID 變數中
    ...
    if (inputID == 0) {
        ...
        // 如果沒有找到該學號的資料，顯示提示訊息
        if (!found) {     //!found 是 「not found」的意思
            printf("該筆資料不存在。\n");
        }
    }
