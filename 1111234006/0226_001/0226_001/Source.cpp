/*0226題目1.將學生結構放入範例程式碼中，根據學號 ID，透過鍵盤將資料寫入檔案中，要記得將上一週產生的空白檔案複製到新的專案目錄中*/
#include <stdio.h>
...
struct clientData {
    ...
    char math, computer;    // 數學成績與計概成績（使用 char 類型儲存 0-100 的分數）
...
int main(void)
{
    ...
    if (fopen_s(&cfPtr, "accounts.dat", "rb+") != 0) {
        ...
        // 要求使用者輸入學號，學號範圍 1 到 100，0 代表結束輸入
        printf("%s", "Enter account number" //輸入學號
        //補充說明帳號必須是 1 到 100 之間，且輸入 0 代表結束輸入。
            " (1 to 100, 0 to end input): "); 
        scanf_s("%d", &client.ID);

        // 當使用者輸入學號不為 0 時，繼續要求輸入學生資料
        while (client.ID != 0) {
            // 要求使用者輸入姓名、數學成績、計概成績、餘額
            printf("%s", "請輸入姓、名和餘額: ");
